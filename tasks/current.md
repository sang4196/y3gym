...
- 상태: 이전 격리 서버 시험 실행·보고 완료. 2026-09-18 후속 검증은 **자동화 도구 BLOCKED / 수동 일부 확인(사용자 보고)**. 최신 범위는 아래 마지막 기록 및 검증 보고서 §13 참조. 이 파일 및 실험 경로가 없었다는 기록은 최초 시험 시작 당시다.
...
- 이후 사용자 보고: A 편집 제목 `SPIKE DRAFT A`, 대표 초록·본문 보라, 대표/본문 두 선택창의 목록·작은 썸네일 표시 확인. 직접 관찰 PASS나 SP-01 전체 PASS는 아님. 다음은 변경 없이 기존 Preview의 제목·초록/보라 표시와 독립 비로그인 창 공개 HTML의 `SPIKE PUBLIC A`·빨강/파랑 표시 확인. seed·preview 소스로 기대값만 확인했다. 서식 편집·업로드·저장·Preview·방문자 비교·보호 접근은 아직 NOT_RUN. 사용자 서버·계정·DB·미디어 유지, Git 쓰기 작업 없음.
- 최신 사용자 보고: 기존 Preview의 초록 대표·보라 본문 표시, 비로그인 시크릿 공개 HTML의 `SPIKE PUBLIC A`·빨강 대표·파랑 본문 표시 확인. Preview 제목은 미확인. 기존 데모 비교이며 새 편집/Save draft 후 유지 검증은 아님. SP-05 전체 PASS·직접 관찰로 확대하지 않음. 다음 사용자 검사: 제목/본문에 TEST 표식 추가 → 하단 Save draft → 재열기·Preview 반영 → 시크릿 공개본에 표식 없는지 확인. 이미지 업로드·서식·JSON 전후·리비전·무권한 파일·JS는 별도 미검증. 이 단계에서 코드·데이터·서버·Git 쓰기 변경 없음.
- 이후 “ㅇㅇ 다 됨” 사용자 보고: 제목 `TEST-DRAFT-01`·본문 `TEST-BODY-01` 추가 및 Save draft, 재열기·Preview 표식 유지, 시크릿 공개 HTML의 기존 제목·빨강/파랑 유지와 표식 미노출 확인. 사용자에 의해 시험 초안이 변경됨. 직접 관찰/DB 조회/전체 SP PASS 아님. 다음은 새 합성 이미지 업로드·선택·초안 저장 비교. wslpath와 fixture 파일 존재만 재확인했고 Windows 선택창 접근은 미확인. 서식/링크·덮어쓰기 UI·JSON/리비전·무권한 접근·JS는 미검증. 에이전트 변경은 이 기록과 보고서뿐이며 서버·DB·미디어·Git 쓰기 작업 없음.
- 최신 사용성 보고: 파랑 대표 변경까지만 완료, 본문 교체는 찾기 어려움·클릭 수·좁은 영역/입력 불편 세 사유로 중단. 저장·Preview·공개본 비교는 미확인. 미저장 가능성이 있어 사용자 서버·브라우저·DB·초안·미디어 자동 변경 없음.
- 제한 개선: Post 패널의 한국어 제목/설명과 본문 위 HelpPanel 템플릿 추가. 기존 이미지 Edit는 형식/alt 수정이라는 설치 소스 근거를 반영. 새 이미지 먼저 삽입 후 기존 배치 제거, 이미 업로드한 자산 선택, Preview 패널 닫기 안내. 기본 단계 수·폭·입력 불량 자체를 해결했다고 주장하지 않음. 자세한 근거/후속 확장 비용은 보고서 §14.
- 검증: 기존 SP-01·03·05 서버 테스트 3개 / 2.850s / OK, 마이그레이션 변경 없음, diff 검사 통과. 새 `.runtime/editor-ux-*/verification.json` 요약 증거. 브라우저 UX 재확인 NOT_RUN. 다음은 사용자가 현재 변경을 Save draft로 보존한 뒤 성공 보고; 이후 자기 --noreload 서버 재시작과 새 화면 안내/교체 흐름 재확인 필요. 에이전트가 서버 재시작·종료·Git 쓰기를 수행하지 않음.
